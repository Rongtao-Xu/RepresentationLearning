...
def calculate_variance_term(pred, gt, means, n_objects, delta_v, norm=2):
    ...
    _var = (torch.clamp(torch.norm((pred - means), norm, 3) , min=0.0) ) * gt[:, :, :, 0]
    var_term = 0.0
    for i in range(bs):
...
def _compute_unlabeled_push2(var_term,cluster_means, pred, gt ,norm,delta_unlabeled):
    ...
    #print(embeddings.shape)(2, 262144, 16)
    #print(cluster_means.shape)(2, 20, 16)
    H = -torch.mul(f.softmax(pred, dim=1),f.log_softmax(pred, dim=1))-torch.mul((1- f.softmax(pred, dim=1)),f.log_softmax(pred, dim=1))
    #H = -pred * torch.log(pred) -(1- pred)* torch.log(1- pred)
    #print(H[:,0])
    H_loss = torch.sum(H[:, 0]) / pred.shape[0]
    # decrease number of instances `C` since we're ignoring 0-label
    n_instances -= 1
    # if there is only 0-label in the target return 0
    if n_instances == 0:
    ...
    # normalize by the number of instances
    return (background_push / n_instances+var_term)** 2 + H_loss
def discriminative_loss(input, target, n_objects,
    ...
    #losses =  beta * dist_term + gamma * reg_term #+ unlabeled_push_weight * unlabeled_push_term
