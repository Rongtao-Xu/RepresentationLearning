...
class GeneralTransformerBlockatt(nn.Module):
    expansion = 1
    ...
    def __init__(
        ...
        super(GeneralTransformerBlockatt, self).__init__()
        self.dim = inplanes
        self.out_dim = planes
        self.num_heads = num_heads
        self.window_size = window_size
    ...
    def forward(self, x,y, mask=None):
        ...
        x = x.view(B, C, -1).permute(0, 2, 1)
        y = y.view(B, C, -1).permute(0, 2, 1)
        xa , att= self.attn(self.norm1(x), self.norm1(y), H, W)
        # Attention
        ...
        # reshape
        x = x.permute(0, 2, 1).view(B, C, H, W)
        #print('att',att.shape)att torch.Size([49, 2888, 32])
        return x,att
