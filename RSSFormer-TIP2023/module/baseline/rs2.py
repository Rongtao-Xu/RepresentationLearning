...
@registry.MODEL.register('rs2NetFusion')
class rsNetFusion(er.ERModule):
    def __init__(self, config):
        ...
        self.neck = ASFF(self.config.neck.in_channels)
...
def add_conv(in_ch, out_ch, ksize, stride, leaky=True):
    ...
    return stage
from .base_hrnet.modules.DAL import MultiheadAttention as MHA

class ASFF(nn.Module):
    def __init__(self, vis=False,**kwargs):
    ...
    def forward(self, feat_list, **kwargs):
        ...
        ca0 = ca0.flatten(2)
        ca0 = ca0.transpose(-1, -2)  # (B, n_patches, hidden)
        B, n_patch, hidden = ca0.size()
        h, w = int((n_patch)), int((n_patch))
        print(ca0.shape)
        out = self.attn(ca0, ca0, ca0,**kwargs)
        x = out.permute(0, 2, 1)
        x = x.contiguous().view(B, 3, h * 2, w * 2)
        print(x.shape)
        out2 = self.attn(ca0, ca0, ca0,**kwargs)
