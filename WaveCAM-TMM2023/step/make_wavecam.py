...
def _work(process_id, model, dataset, args):
    ...
    wavecam_predictor.load_state_dict(torch.load(osp.join(args.wavecam_weight_dir,'wavecam_predictor_'+str(args.wavecam_num_epoches) + '.pth')))

    with torch.no_grad(), cuda.device(process_id):

        model.cuda()
        wavecam_predictor.cuda()
        for iter, pack in enumerate(data_loader):
            ...
            outputs = [model.forward2(img[0].cuda(non_blocking=True),wavecam_predictor.classifier.weight) for img in pack['img']] # b x 20 x w x h
...
def run(args):
    ...
    model.load_state_dict(torch.load(osp.join(args.wavecam_weight_dir,'res50_wavecam_'+str(args.wavecam_num_epoches) + '.pth')))
    model.eval()
