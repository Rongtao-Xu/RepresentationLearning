...
from utils.losses import DenseEnergyLoss, get_aux_loss, get_energy_loss
...
from network.TSCD_model import TSCD
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    model.train()
    return cls_score, seg_score, cam_score, ref_score
...
def train(cfg):
    ...
    tscd = TSCD(backbone=cfg.backbone.config,
    ...
    logging.info('\nNetwork config: \n%s' % (tscd))
    param_groups = tscd.get_param_groups()
    ...
    if args.local_rank == 0:
        ...
        # writer.add_graph(network, dummy_input)
    ...
    logging.info('\nOptimizer: \n%s' % optimizer)
    tscd = DistributedDataParallel(tscd, device_ids=[args.local_rank], find_unused_parameters=True)
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        cls, segs, attns, attn_pred = tscd(inputs, seg_detach=args.seg_detach)

        cams, ref_mat = multi_scale_cam_with_ref_mat(tscd, inputs=inputs, scales=cfg.cam.scales)
        ...
        cls2, segs2, attns2,attn_pred2 = tscd(img2, seg_detach=args.seg_detach)
        cams2 = multi_scale_cam(tscd, inputs=img2, scales=cfg.cam.scales)
        ...
        refined_pseudo_label = refine_cams_with_bkg_v2(varm, inputs_denorm, cams=cams, cls_labels=cls_labels, cfg=cfg,
                                                      img_box=img_box)
        ref_label = cams_to_refine_label(refined_pseudo_label, mask=attn_mask, ignore_index=cfg.dataset.ignore_index)
        aux_loss, pos_count, neg_count = get_aux_loss(attn_pred, ref_label)



        refined_ref_label = refined_pseudo_label

        seg_loss = get_seg_loss(segs, refined_ref_label.type(torch.long), ignore_index=cfg.dataset.ignore_index)
        reg_loss = get_energy_loss(img=inputs, logit=segs, label=refined_ref_label, img_box=img_box,
        ...
        if n_iter <= cfg.train.cam_iters:
            loss = 1.0 * cls_loss + 0.0 * seg_loss + 0.0 * reg_loss + 0.0 * (aux_loss)
        else:
            loss = 1.0 * cls_loss + 0.1 * seg_loss + 0.01 * reg_loss + 0.1 * (aux_loss) + 0.1 * loss_corr + 0.1 * loss_er

        avg_meter.add({'cls_loss': cls_loss.item(), 'seg_loss': seg_loss.item(), 'aux_loss': aux_loss.item(),
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info(
                    "Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, aux_loss: %.4f, corr_loss: %.4f, pseudo_seg_loss %.4f, pseudo_seg_mAcc: %.4f" % (
                    n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('aux_loss'),avg_meter.pop('corr_loss'), avg_meter.pop('seg_loss'), seg_mAcc))
                ...
                writer.add_image("train/ref_gts", grid_ref_gt, global_step=n_iter)
        ...
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ckpt_name = os.path.join(cfg.work_dir.ckpt_dir, "tscd_iter_%d.pth" % (n_iter + 1))
            if args.local_rank == 0:
                logging.info('Validating...')
                torch.save(tscd.state_dict(), ckpt_name)
            cls_score, seg_score, cam_score, ref_score = validate(model=tscd, data_loader=val_loader, cfg=cfg)
            if args.local_rank == 0:
