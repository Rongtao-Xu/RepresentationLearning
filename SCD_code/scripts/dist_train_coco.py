...
def train(cfg):
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        if n_iter <= cfg.train.cam_iters:
            ...
            loss = 1.0 * cls_loss + 0.1 * seg_loss + 0.01 * reg_loss + 0.1 * (
            aux_loss) + 0.1 * loss_corr + 0.1 * loss_er
            ...
            if args.local_rank == 0:
                logging.info(
                    "Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, aux_loss: %.4f, corr_loss: %.4f, pseudo_seg_loss %.4f, pseudo_seg_mAcc: %.4f" % (
                        n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('aux_loss'),
                        avg_meter.pop('corr_loss'), avg_meter.pop('seg_loss'), seg_mAcc))
